...
for j=DynamicRegressionLength:NMonth
    ...
    % auto filter by excluding exposures with >0.5 p-value
    PValFilterInitial=0.1;
    while isempty(DoubleFilteredFactorID{j})
        for k=1:length(UserFilteredFactorID)
    ...
    if exist('HWMDynamicSelectedFactorID','var')
        ...
        DynamicLengthSelectedFactors{j}=HWMDynamicSelectedFactors;
...
for StaticGroup=1:1
    ...
    annotation('textbox', [0.2,0.4,0.1,0.1],...
           'String', ['Static Adjusted R-Squared',AdjR2]);
