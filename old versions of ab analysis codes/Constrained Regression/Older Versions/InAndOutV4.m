...
InCorrcoef=corrcoef([InFactorPortfolioReturnDist{NFactors},TargetFund(1:NinSample,:)]);

OutFactorPortfolioReturnDist{NFactors}= Factors(NinSample+1:end,:)*CoefficientsDistGlobalFormat{NFactors};
OutVolRatioDistStacked{NFactors}=repmat(std(TargetFund(NinSample+1:end))./std(OutFactorPortfolioReturnDist{NFactors}),[NMonth-NinSample,1]);
OutVolAdjAlphaDist{NFactors}=mean(TargetFund(NinSample+1:end))-mean(OutVolRatioDistStacked{NFactors}.*OutFactorPortfolioReturnDist{NFactors});
OutCorrcoef=corrcoef([OutFactorPortfolioReturnDist{NFactors},TargetFund(1+NinSample:end,:)]);

FullFactorPortfolioReturnDist{NFactors}= Factors*CoefficientsDistGlobalFormat{NFactors};
FullVolRatioDistStacked{NFactors}=repmat(std(TargetFund)./std(FullFactorPortfolioReturnDist{NFactors}),[NMonth,1]);
FullVolAdjAlphaDist{NFactors}=mean(TargetFund)-mean(FullVolRatioDistStacked{NFactors}.*FullFactorPortfolioReturnDist{NFactors});
FullCorrcoef=corrcoef([FullFactorPortfolioReturnDist{NFactors},TargetFund]);
...
CumRatioAtNinSample=(1+CumTargetFund(NinSample))/(1+CumSameVolFactorPortfolio(NinSample));
plot(Dates(NinSample:end),(1+CumSameVolFactorPortfolio(NinSample:end))*CumRatioAtNinSample-1,'-r');
...
ylim([min([CumTargetFund;CumSameVolFactorPortfolio])-0.1,max([CumTargetFund;(1+CumSameVolFactorPortfolio(NinSample:end))*CumRatioAtNinSample-1])+0.1]);
...
hx=get(Bar2(1),'XData'); 
    hy=get(Bar2(1),'YData');
    text(hy(1), hx(1), num2str(hy(1))); 
    
% R2 vs alpha
subplot(2,3,4);
hold on;
for i=NFactors:NFactors
    scatter(InVolAdjAlphaDist{i}*12,R2Dist{i},'.b');
    scatter(OutVolAdjAlphaDist{i}*12,OutCorrcoef(end,1:(end-1)).^2,'.r');
end
