...
while AdjR2==0
    for NFactors=1:min(NMax,length(DoubleFilteredFactorIDStatic))
        ...
        if 1==1
            ...
            CoefficientsFilter{NFactors}=(max(abs(CoefficientsDist{NFactors}(2:end,:)),[],1)<CoefficientThreshold);               % test if any factor in each trial has coefficients >1 or <-1
            AdjR2Dist{NFactors}(1,:)=AdjR2Dist{NFactors}(1,:).*PValFilter{NFactors}.*CoefficientsFilter{NFactors};                % assign 0 to AdjR2 if the trial has any factor with insignificant p-values or large coefficients
        end
        ...
        if sum(abs(AdjR2Dist{NFactors}))~=0                                                         % if any trial's factor exposures are all significant
            ...
            TempYhat{NFactors}=YhatDist{NFactors}(:,TempSelectedTrial{NFactors});
        ...
        if nnz(AdjR2Dist{NFactors})>1                                                               % if two or more trials satisfied the constraints
            ...
            SecondFactorNames{NFactors}=FilteredFactorNames(:,FactorIDCombo{NFactors}(SecondTrial{NFactors},:)); 
        ...
        if nnz(AdjR2Dist{NFactors})>2                                                               % if three or more trials satisfied the constraints
            ...
            ThirdFactorNames{NFactors}=FilteredFactorNames(:, FactorIDCombo{NFactors}(ThirdTrial{NFactors},:));
...
text(-MaxAbsReturn,MaxAbsReturn,[' R-Square: ',num2str(R2,'%.4f')],...                              % Dsiplay R-square
...
subplot(2,3,4);
x=[1:3]';
y=[ThirdMaxadjR2;SecondMaxadjR2;AdjR2];
...
set(Ledgend2,'Location','SouthWest','FontSize',6)
set(Ledgend2,'color','none');
set(gca,'XLim',[min(Dates),max(Dates)+100]);
...
set(Ledgend3,'Location','SouthWest','FontSize',6)
set(Ledgend3,'color','none');
set(gca,'YScale','log')
...
PlotPoint=1;
PlotID=1;
while all([PlotPoint <size(CumFactors,2),PlotID<=5])
    subplot(2,3,PlotID+1);
    hold on;
    plot(Dates,CumFactors(:,PlotPoint:min(PlotPoint+4,size(CumFactors,2))),'-');
    datetick('x','yy');
    title(['Factor Cumulative Returns, Group ',num2str(PlotID)+1]);
    xlabel('Time (Year)')
    ylabel('Cumulative Returns' )
    Ledgend2=legend(FactorNames{PlotPoint:min(PlotPoint+4,size(CumFactors,2))});
    set(Ledgend2,'Location','NorthWest')
    set(Ledgend2,'color','none');
    set(gca,'XLim',[min(Dates),max(Dates)+100]);
...
% print(OtherFigures,'-dpdf',[cell2mat(FundNames(TargetFundID)),'-OtherCharts']);