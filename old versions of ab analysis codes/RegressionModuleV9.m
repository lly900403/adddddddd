...
SelectedTrial=cell(1,NMax); % declare variable
...
for j=DynamicRegressionLength:NMonth
    ...
    DynamicMonthlyFactors=SelectedFactors(j-DynamicRegressionLength+1:j,:);                     % choose X-month's factor returns
    DynamicMonthlyFund=TargetFund(j-DynamicRegressionLength+1:j);                               % choose X-month's fund returns
    DynamicStatsDist(j)=regstats(DynamicMonthlyFund,DynamicMonthlyFactors,'linear',whichstats); % regress
    DynamicCoefficients(j,:)=DynamicStatsDist(j).tstat.beta';                                   % assign coefficients
    DynamicPVal(j,:)=DynamicStatsDist(j).tstat.pval';                                           % assign p-values
    DynamicAdjR2(j)=DynamicStatsDist(j).adjrsquare;                                             % assign adjusted r-squared
    
    RegressionPeriodFactorPortfolio=DynamicMonthlyFactors*DynamicCoefficients(j,2:end)';        % factor portfolio return for the X-month regression period
    
    DynamicFundVol(j-DynamicRegressionLength+1:j)=std(DynamicMonthlyFund);                      % fund's vol in the X-month regression period
    ...
    SameVolDynamicCoefficients(j,:)=DynamicCoefficients(j,:)*std(DynamicMonthlyFund)/std(RegressionPeriodFactorPortfolio);  % divide coefficients by vol ratio
